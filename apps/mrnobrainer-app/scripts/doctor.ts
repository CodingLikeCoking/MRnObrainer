...
export function collectDoctorResults(): CheckResult[] {
  ...
  const browserSmokeRuntime = detectBrowserSmokeRuntime(appRoot);
  results.push({
    label: "Browser smoke runtime",
    ok: browserSmokeRuntime.ok,
    detail: browserSmokeRuntime.hasLocalPlaywrightBinary
      ? "Local playwright package is installed for browser-smoke execution."
      : "Run bun install to add the local playwright package, then run `npx playwright install chromium`.",
    severity: "error",
  });
...
function main() {
  const results = collectDoctorResults();
  ...
  console.log(
    "Doctor verifies environment and operator wiring only. Use browser smoke, canary, and ship artifacts for release evidence."
  );
}
