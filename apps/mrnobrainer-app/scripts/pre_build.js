...
async function findPosixDownloadTool() {
	const tool = resolvePreferredDownloadTool({
		hasWget: await hasCommand('wget'),
		hasCurl: await hasCommand('curl'),
	});

	if (!tool) {
		console.error('Neither wget nor curl was found. Please install one of them and make sure it is in your PATH.');
		process.exit(1);
	}

	console.log(`using ${tool} for pre-build downloads`);
	return tool;
}

async function downloadWithTool(tool, url, outputPath) {
	const [command, ...args] = buildDownloadCommand({
		tool,
		url,
		outputPath,
	});
	...
	if (exitCode !== 0) {
		throw new Error(`${command} failed while downloading ${url}`);
	}
}
...
if (platform == 'linux') {
	...
	if (!(await fs.exists(config.ffmpegRealname))) {
		const downloadTool = await findPosixDownloadTool()
		await downloadWithTool(downloadTool, config.linux.ffmpegUrl, `${config.linux.ffmpegName}.tar.xz`)
	...
	if (!(await fs.exists(config.linux.tesseractName))) {
		const downloadTool = await findPosixDownloadTool()
		await downloadWithTool(downloadTool, config.linux.tesseractUrl, config.linux.tesseractName)
...
if (platform == 'macos') {
  ...
  if (!(await fs.exists(`ffmpeg-aarch64-apple-darwin`))) {
    await downloadWithTool(downloadTool, config.macos.ffmpegUrlArm, 'ffmpeg-aarch64.zip');
  ...
  if (!(await fs.exists(`ffprobe-aarch64-apple-darwin`))) {
    await downloadWithTool(downloadTool, config.macos.ffprobeUrlArm, 'ffprobe-aarch64.zip');
  ...
  if (!(await fs.exists(`ffmpeg-x86_64-apple-darwin`))) {
    await downloadWithTool(downloadTool, config.macos.ffmpegUrlx86_64, 'ffmpeg-x86_64.zip');
  ...
  if (!(await fs.exists(`ffprobe-x86_64-apple-darwin`))) {
    await downloadWithTool(downloadTool, config.macos.ffprobeUrlx86_64, 'ffprobe-x86_64.zip');
