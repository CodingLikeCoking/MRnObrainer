      ...
      if (data.role === "admin" && key) {
        try {
          ...
          if (tokenRes.ok) {
            ...
            inviteLink = `mrnobrainer://join-team?team_id=${data.team.id}&key=${encodeURIComponent(b64Key)}&invite_token=${tokenData.invite_token}`;
          }
        } catch {
          // fall back to link without token (won't work for joining, but shows the key)
          const b64Key = await exportTeamKey(key);
          inviteLink = `mrnobrainer://join-team?team_id=${data.team.id}&key=${encodeURIComponent(b64Key)}`;
        }
      }
