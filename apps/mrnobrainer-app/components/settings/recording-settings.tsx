  ...
  return (
    <Card className="border-border bg-card">
      <CardContent className="px-3 py-2.5">
        ...
        >
          <Input name="vocab-word" placeholder="e.g. codex" className="h-7 text-xs flex-1" spellCheck={false} autoCorrect="off" autoCapitalize="off" />
...
export function RecordingSettings() {
  ...
  const {
    verificationState: captureVerificationState,
    permissions: capturePermissions,
    health: captureStatusHealth,
    refresh: refreshCaptureStatus,
  ...
  useEffect(() => {
    const unlisten = listen("data-dir-fallback", () => {
      toast({
        ...
        description:
          "the configured data directory could not be accessed. recordings are using the default directory (~/.mrnobrainer).",
        variant: "destructive",
        duration: 10000,
      });
    });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "MRnObrainer has been restarted with the new settings",
      });
    } catch (error) {
...
Your screen is a pipe. Everything you see, hear, and type flows through it. MRnObrainer just makes sure nothing valuable leaks away.`;
  ...
  return (
    <div className="space-y-5">
      ...
      <CaptureHealthCard compact title="Capture health" />

      {!settings.disableVision && captureVerificationState !== "verified" && (
        <Alert
          variant={captureVerificationState === "missing" ? "destructive" : "default"}
        >
          <AlertCircle className="h-4 w-4" />
          <AlertTitle>Screen capture needs attention</AlertTitle>
          <AlertDescription className="space-y-3">
            <p>
              {captureVerificationState === "missing"
                ? "Screen recording is enabled in MRnObrainer, but macOS has not granted screen access yet."
                : captureStatusHealth?.frame_status === "stale"
                  ? "Screen access is granted, but the capture stream looks stale. Restart capture to rebind the recorder."
                  : "Screen access is granted. If the capture cards stay empty, restart the recorder from here."}
            </p>
              ...
              {captureVerificationState === "missing" ? (
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        toast({
                          title: "watching for screen access",
                          description:
                            "MRnObrainer will relaunch automatically once macOS grants screen access",
                        });
                      } catch (error) {
                        toast({
                          title: "could not request screen access",
                ...
                <>
                  <Button
                    ...
                    disabled={isUpdating || Object.keys(validationErrors).length > 0}
                  >
                    {hasUnsavedChanges ? "Apply & Restart" : "Restart capture"}
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
            ...
            {capturePermissions?.screenRecording === "granted" &&
              captureStatusHealth?.frame_status === "not_started" && (
                <p className="text-xs text-muted-foreground">
                  Screen access is granted, but no frames have arrived yet.
                  Restarting the recorder will rebind the capture pipeline.
                </p>
              )}
          </AlertDescription>
        </Alert>
          ...
          <CardContent className="px-3 py-2.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                ...
                <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                  Your name
                  <HelpTooltip text="Your name in transcripts. Click 'train' and speak for 30 seconds to teach MRnObrainer your voice — it will recognize you across all devices using voice matching." />
                </h3>
              </div>
              <div className="flex items-center gap-1.5">
                ...
                <SelectContent>
                  <SelectGroup>
                    ...
                    <SelectItem value="screenpipe-cloud" disabled={!settings.user?.cloud_subscribed}>
                      Managed Cloud {!settings.user?.cloud_subscribed && "(login required)"}{hwCapability?.recommendedEngine === "screenpipe-cloud" && " ★"}
                    </SelectItem>
              ...
              <div className="mt-2 ml-[26px] p-2 rounded-md bg-yellow-500/10 border border-yellow-500/30">
                <p className="text-xs text-yellow-600 dark:text-yellow-400">
                  ...
                  {settings.user?.cloud_subscribed
                    ? " Consider switching to the cloud engine for better performance."
      ...
      <Dialog open={voiceTraining.dialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            speak naturally at your normal pace — this helps MRnObrainer learn your voice
          </DialogDescription>
