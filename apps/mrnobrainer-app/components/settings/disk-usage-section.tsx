...
import { RefreshCw, HardDrive, Folder, Video, Database, Calculator, FileText } from "lucide-react";
import { Skeleton } from "../ui/skeleton";
import { cn } from "@/lib/utils";
import { Progress } from "../ui/progress";
import { summarizeStoragePace } from "@/lib/product-config";

export function DiskUsageSection() {
  const { diskUsage, isLoading, error, refetch } = useDiskUsage();
  const storagePace = summarizeStoragePace({
    recordingSince: diskUsage?.recording_since,
    totalDataBytes: diskUsage?.total_data_bytes,
  });
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Monitor storage usage for your local MRnObrainer data
          </p>
        </div>
        <Card>
    ...
    <div className="space-y-5">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground text-sm">
          Storage usage at the local capture folder `~/.mrnobrainer`
        </p>
      </div>
      ...
      {!isLoading && (
        <Card className="border-border bg-card">
          <CardContent className="px-3 py-2.5">
            <p className="text-sm font-medium text-foreground">Observed storage pace</p>
            <p className="mt-1 text-xs text-muted-foreground">
              {storagePace
                ? `Based on your current history, this device is using about ${storagePace.gbPerWorkday.toFixed(1)} GB for an 8-hour workday and ${storagePace.gbPerDay.toFixed(1)} GB for a full day of continuous capture.`
                : "Use MRnObrainer for a bit longer and this section will estimate an 8-hour workday footprint from your own history."}
            </p>
          </CardContent>
        </Card>
      )}
          ...
          {isLoading ? (
            ...
            <div className="space-y-1.5 ml-[26px]">
              ...
              {diskUsage?.other?.logs_size?.includes("GB") && (
                <p className="text-[11px] text-destructive mt-1">Logs are large. Delete old ones from the MRnObrainer logs folder.</p>
              )}
            </div>
          )}
        </CardContent>
