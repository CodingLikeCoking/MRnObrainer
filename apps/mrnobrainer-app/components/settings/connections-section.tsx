...
import { AndroidPairingCard } from "./android-pairing-card";
...
async function getLatestMcpRelease(): Promise<{ url: string; version: string }> {
  ...
  const maxPages = 5;
  for (const apiBase of GITHUB_RELEASES_APIS) {
    for (let page = 1; page <= maxPages; page++) {
      const response = await tauriFetch(`${apiBase}?per_page=50&page=${page}`, {
        method: "GET",
        headers: {
...
async function getInstalledMcpVersion(): Promise<string | null> {
  try {
    ...
    if (config?.mcpServers?.mrnobrainer || config?.mcpServers?.screenpipe) {
      return "installed";
    }
...
async function isCursorMcpInstalled(): Promise<boolean> {
  try {
    ...
    return !!config?.mcpServers?.mrnobrainer || !!config?.mcpServers?.screenpipe;
  } catch {
    return false;
  }
}
...
async function installCursorMcp(): Promise<void> {
  ...
  (config.mcpServers as Record<string, unknown>).mrnobrainer = {
    command: "npx",
...
function CursorCard() {
  ...
  const handleConnect = async () => {
    try {
      ...
      await message(
        "Failed to write Cursor MCP config.\n\nYou can manually add MRnObrainer to ~/.cursor/mcp.json:\n\n" +
        JSON.stringify({ mcpServers: { mrnobrainer: { command: "npx", args: ["-y", "screenpipe-mcp"] } } }, null, 2),
          ...
          <div className="px-4 pb-4">
            <div className="p-3 bg-muted border border-border rounded-lg">
              <p className="text-xs text-foreground">
                <strong>MRnObrainer is now connected to Cursor.</strong> Try asking in Composer:
...
const CLAUDE_CODE_COMMAND = "claude mcp add mrnobrainer -- npx -y screenpipe-mcp";
...
export function ConnectionsSection() {
  ...
  const handleClaudeConnect = async () => {
    try {
      ...
      const mcpbPath = await join(tmp, "mrnobrainer.mcpb");
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
          ...
          <CardContent className="p-0">
            ...
            {downloadState === "downloaded" && (
              <div className="px-4 pb-4">
                <div className="p-3 bg-muted border border-border rounded-lg">
                  <p className="text-xs text-foreground">
                    <strong>MRnObrainer is now connected to Claude.</strong> If Claude was already running, restart it to pick up the changes. Try asking: &quot;What did I do in the last 5 minutes?&quot;
                  </p>
                </div>
              </div>
            )}
        ...
        {/* Docs link */}
        <button
          onClick={() => openUrl(MRNOBRAINER_GITHUB_README_URL)}
