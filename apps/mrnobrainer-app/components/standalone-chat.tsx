...
DEEP LINKS & MEDIA:
- Frame (PREFERRED): [10:30 AM — Chrome](mrnobrainer://frame/12345) — use frame_id from OCR search results. NEVER invent frame IDs.
- Timeline (audio only): [meeting at 3pm](mrnobrainer://timeline?timestamp=2024-01-15T15:00:00Z) — use exact timestamp from audio search results.
- Video: show .mp4 paths in inline code: \`/path/to/video.mp4\`
ALWAYS use mrnobrainer://frame/{frame_id} for OCR results. Only use mrnobrainer://timeline for audio results (no frame_id). Copy IDs/timestamps verbatim from search results — NEVER fabricate them.
...
function MarkdownBlock({ text, isUser }: { text: string; isUser: boolean }) {
  return (
    <MemoizedReactMarkdown
      ...
      urlTransform={preserveAllowedChatDeepLinks}
      components={{
        p({ children }) {
        ...
        a({ href, children, ...props }) {
          ...
          if (isChatDeepLink(href)) {
            const handleTimelineClick = async (e: React.MouseEvent<HTMLAnchorElement>) => {
              e.preventDefault();
              try {
                const deepLinkTarget = parseChatDeepLinkTarget(href);
                if (!deepLinkTarget) {
                  throw new Error("Invalid chat deep link");
                }

                if (deepLinkTarget.kind === "frame") {
                  const { frameId } = deepLinkTarget;
                  if (frameId) {
                ...
                const { timestamp } = deepLinkTarget;
    ...
    <div className={cn("flex flex-col bg-background", className ?? "h-screen")}>
      ...
      >
        ...
        <div className="flex-1">
          <h2 className="font-semibold text-sm tracking-tight">MRnObrainer Assistant</h2>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {messages.length === 0 && disabledReason && (!hasPresets || !hasValidModel) && (
        ...
        {messages.length === 0 && hasPresets && hasValidModel && (
          <SummaryCards
