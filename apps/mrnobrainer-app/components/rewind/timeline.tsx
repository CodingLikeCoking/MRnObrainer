...
export type TimelineInteractionMode = "page" | "focus";
...
export default function Timeline({
	...
	const {
		permissions,
		verificationState,
		refresh: refreshCaptureSnapshot,
	} = useCaptureStatusSnapshot(4000);
	...
	const [crossDeviceEvents, setCrossDeviceEvents] = useState<CrossDeviceTimelineEvent[]>([]);
	const [crossDeviceEventsLoading, setCrossDeviceEventsLoading] = useState(false);
	...
	useEffect(() => {
		...
		const loadCrossDeviceEvents = async () => {
			setCrossDeviceEventsLoading(true);
			try {
				const url = new URL("http://localhost:3030/search");
				url.searchParams.set("content_type", "input");
				url.searchParams.set("limit", "200");
				url.searchParams.set("start_time", startOfDay(currentDate).toISOString());
				url.searchParams.set("end_time", endOfDay(currentDate).toISOString());
				...
				if (!response.ok) {
					throw new Error(`failed to load cross-device events: ${response.status}`);
				}

				const json = await response.json();
				const rows = Array.isArray(json?.data)
					? json.data.map((item: { content?: Record<string, unknown> }) => item?.content ?? item)
					: [];

				setCrossDeviceEvents(normalizeCrossDeviceTimelineEvents(rows));
			} catch (error) {
				if (controller.signal.aborted) {
					return;
				}
				console.error("failed to load cross-device timeline events", error);
				setCrossDeviceEvents([]);
			} finally {
				if (!controller.signal.aborted) {
					setCrossDeviceEventsLoading(false);
				}
			}
		};

		loadCrossDeviceEvents();
	...
	const timelineStatus = getTimelineStatusState({
	...
	const shouldShowStatusCard =
		!currentFrame && frames.length === 0 && timelineStatus.kind !== "none";
	const shouldShowTimelineChrome = !shouldShowStatusCard;
	...
	const handleRestartBackend = useCallback(async () => {
		...
		try {
			...
			await refreshCaptureSnapshot();
			toast({
				title: "capture backend restarted",
				description: "The local recorder is starting again in the background.",
			});
		} catch (restartError) {
			toast({
				title: "could not restart capture backend",
				description:
					restartError instanceof Error
						? restartError.message
						: "Open Recording settings to inspect the recorder.",
				variant: "destructive",
			});
		} finally {
	...
	const runPrimaryStatusAction = useCallback(async () => {
		switch (timelineStatus.primaryAction) {
			case "open_permissions":
				try {
					...
					toast({
						...
						description: String(permissionError),
						variant: "destructive",
					});
				}
				return;
	...
	}, [handleRestartBackend, refreshCaptureSnapshot, timelineStatus.primaryAction]);
	...
	useTimelineKeyboard({
		enabled: !embedded || interactionMode === "focus",
		frames,
		currentIndex,
		setCurrentIndex,
		currentFrame,
			...
			>
				...
				<div className={`absolute inset-0 z-10 ${embedded ? "bg-background" : "bg-black"}`}>
					...
					{shouldShowStatusCard ? (
						<div className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-background via-background to-muted/20 px-4 py-6">
							<div className="w-full max-w-xl rounded-[28px] border border-border/70 bg-background/92 p-6 text-left shadow-[0_24px_80px_rgba(15,23,42,0.10)] backdrop-blur-xl sm:p-7">
								<div className="flex flex-col gap-5">
									<div className="flex items-start gap-4">
										<div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-border/70 bg-muted/40">
											{timelineStatus.kind === "permission_blocked" ? (
												<MonitorOff className="h-5 w-5 text-amber-600" />
											) : timelineStatus.kind === "backend_offline" ? (
												<AlertCircle className="h-5 w-5 text-rose-600" />
											) : timelineStatus.kind === "loading" ? (
									...
									<div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
										<span className="rounded-full border border-border/60 bg-muted/30 px-3 py-1">
											Screen Recording: {permissions?.screenRecording ?? "unknown"}
										</span>
										<span className="rounded-full border border-border/60 bg-muted/30 px-3 py-1">
											Recorder: {isServerDown ? "offline" : health?.frame_status || "starting"}
										</span>
										<span className="rounded-full border border-border/60 bg-muted/30 px-3 py-1">
									...
									<div className="flex flex-wrap items-center gap-2">
										{timelineStatus.primaryAction ? (
											<Button
												onClick={runPrimaryStatusAction}
												disabled={isRestartingBackend}
												className="rounded-full"
											>
												{timelineStatus.primaryAction === "open_permissions"
													? "Open Screen Recording settings"
													: timelineStatus.primaryAction === "restart_backend"
														? isRestartingBackend
															? "Restarting capture backend..."
															: "Restart capture backend"
														: "Re-check"}
											</Button>
										) : null}
										<Button
											variant="outline"
											onClick={() => commands.showWindow({ Settings: { page: "recording" } })}
											className="rounded-full"
										>
				...
				{!embedded && shouldShowTimelineChrome && <div className="absolute top-0 left-0 right-0 h-20 bg-gradient-to-b from-black/20 via-black/5 to-transparent z-30 pointer-events-none" />}

				{/* Bottom Gradient Overlay - Very subtle, only where timeline is */}
				{!embedded && shouldShowTimelineChrome && <div className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-black/10 via-black/2 to-transparent z-30 pointer-events-none" />}

				{/* Top Controls */}
				{shouldShowTimelineChrome ? (
				<div className={`absolute top-0 left-0 right-0 z-40 px-4 pb-4 ${embedded ? "pt-3" : "pt-[calc(env(safe-area-inset-top)+16px)]"}`}>
					<TimelineControls
				...
				{shouldShowTimelineChrome ? (
				<div className={`absolute ${embedded ? "top-14" : "top-[calc(env(safe-area-inset-top)+56px)]"} right-4 z-40`}>
				...
				<div className="absolute bottom-0 left-0 right-0 z-40 pointer-events-auto">
					{crossDeviceEventsLoading ? null : crossDeviceEvents.length > 0 ? (
						<div className="px-3 pb-3">
							<CrossDeviceActivityLane events={crossDeviceEvents} />
						</div>
					) : null}
					{frames.length > 0 ? (
						<TimelineSlider
