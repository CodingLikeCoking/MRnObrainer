...
export function useScrollZoom(opts: {
		...
		() =>
			throttle(
				(e: WheelEvent) => {
					...
					if ((e.ctrlKey || e.metaKey) && !modifierDrivenScroll) {
	...
	[
		...
		embedded,
		interactionMode,
	], // Re-create when zoom/filter changes
	);
	...
	useEffect(() => {
		if (embedded && interactionMode === "page") {
			return;
		}
	...
	}, [handleScroll, embedded, interactionMode]);
