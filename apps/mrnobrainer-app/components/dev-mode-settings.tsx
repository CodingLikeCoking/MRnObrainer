...
const getDebuggingCommands = (os: string | null, dataDir: string) => {
  ...
  if (os === "windows") {
    ...
    cliInstructions =
      "# 1. Open Terminal\n# 2. Navigate to: /Applications/MRnObrainer.app/Contents/MacOS/\n#    Type: cd /Applications/MRnObrainer.app/Contents/MacOS/\n";
  } else if (os === "linux") {
    cliInstructions =
  ...
  const baseInstructions = `# First, view the ${PRODUCT_NAME} capture CLI arguments:
  ${cliInstructions}
  # 3. Run: ./mrnobrainer-app --help
  # 4. Choose your preferred setup and start capture manually:
  #    (Replace [YOUR_ARGS] with your chosen arguments)
  #    Example: ./mrnobrainer-app `;
  ...
  const baseCommand =
    ...
    (os === "windows"
      ...
      : "") +
    "\n\n# 5. If you've already started the capture backend, try these debugging commands:\n";
...
export const DevModeSettings = ({ localDataDir }: { localDataDir: string }) => {
  ...
  const handleStartScreenpipe = async () => {
    setIsLoading(true);
    const toastId = toast({
      title: `starting ${PRODUCT_NAME} capture`,
    ...
    try {
      ...
      toastId.update({
        id: toastId.id,
        title: `${PRODUCT_NAME} capture started`,
        description: `${PRODUCT_NAME} capture is now running.`,
        duration: 3000,
      });
    } catch (error) {
      ...
      toastId.update({
        id: toastId.id,
        title: "error",
        description: `failed to start ${PRODUCT_NAME} capture.`,
        variant: "destructive",
        duration: 3000,
      });
    } finally {
  ...
  const handleStopScreenpipe = async () => {
    setIsLoading(true);
    const toastId = toast({
      title: `stopping ${PRODUCT_NAME} capture`,
      description: "please wait...",
      duration: Infinity,
    });
    try {
      ...
      toastId.update({
        id: toastId.id,
        title: `${PRODUCT_NAME} capture stopped`,
        description: `${PRODUCT_NAME} capture is now stopped.`,
        duration: 3000,
      });
    } catch (error) {
      ...
      toastId.update({
        id: toastId.id,
        title: "error",
        description: `failed to stop ${PRODUCT_NAME} capture.`,
        variant: "destructive",
        duration: 3000,
      });
    } finally {
            ...
            <Card className="p-8">
                  ...
                  <div className="flex flex-col items-center w-full">
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>stop the MRnObrainer capture backend</p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  </div>
              ...
              <CardFooter className="flex flex-col items-center">
                <p className="text-sm text-muted-foreground">
                  manually start or stop MRnObrainer capture
                </p>
