...
import { Check, Monitor, Mic, Keyboard, Globe, AlertTriangle, Upload, Loader } from "lucide-react";
...
import { useSettings, DEFAULT_PROMPT, getDefaultAIPreset } from "@/lib/hooks/use-settings";
...
const OnboardingStatus: React.FC<OnboardingStatusProps> = ({
  ...
  const ensureDefaultPreset = async () => {
    if (settings.aiPresets.length === 0) {
      await updateSettings({ aiPresets: [getDefaultAIPreset() as any] });
    }
  };
  ...
  return (
    <div className={`${className} w-full flex flex-col items-center justify-center min-h-[400px]`}>
      ...
      >
        ...
        <h1 className="font-mono text-base font-bold text-foreground">MRnObrainer</h1>
      </motion.div>
