...
import { getDefaultAIPreset, useSettings } from "@/lib/hooks/use-settings";
...
import { LOCAL_API_BASE_URL, PRODUCT_CLOUD_BASE_URL } from "@/lib/product-config";
  ...
  useEffect(() => {
    const start = async () => {
      try {
        ...
        const healthCheck = await fetch(`${LOCAL_API_BASE_URL}/health`, {
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center min-h-[400px]">
      ...
      >
        ...
        <img className="w-12 h-12 mb-2" src="/128x128.png" alt="MRnObrainer" />
        ...
        <AnimatePresence>
          {state === "stuck" && (
            ...
            >
              ...
              >
                continue without verified capture →
              </button>
                ...
                <Button
                  ...
                  >
                    ...
                    <>
                      ...
                      <>
                      <Upload className="w-3 h-3 mr-1" /> share logs
                      </>
                    )}
                  </Button>
              </div>
              <p className="max-w-xs text-center font-mono text-[10px] leading-relaxed text-muted-foreground/70">
                Log sharing is optional and uploads recent startup logs to MRnObrainer support so we can debug this device.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
