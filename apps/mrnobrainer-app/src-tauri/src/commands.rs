...
#[tauri::command]
#[specta::specta]
pub fn get_default_settings_store() -> crate::store::SettingsStore {
    crate::store::SettingsStore::default()
}
...
#[cfg(target_os = "windows")]
const DEFAULT_SHORTCUT: &str = "Alt+Shift+O";
#[cfg(not(target_os = "windows"))]
const DEFAULT_SHORTCUT: &str = "Shift+Super+O";
...
pub async fn open_login_window(app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        .title(&format!("sign in to {}", crate::brand::PRODUCT_NAME))
        .inner_size(460.0, 700.0)
        .focused(true)
        .on_navigation(move |url| {
            if matches!(url.scheme(), "screenpipe" | "mrnobrainer") {
...
#[tauri::command]
#[specta::specta]
pub async fn set_onboarding_mode(app_handle: tauri::AppHandle, mode: String) -> Result<(), String> {
    OnboardingStore::update(&app_handle, |onboarding| {
        onboarding.mode = Some(mode);
    })?;
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub async fn restart_after_update(app_handle: tauri::AppHandle) -> Result<(), String> {
    if let Some(manager) = app_handle.try_state::<std::sync::Arc<crate::updates::UpdatesManager>>() {
        manager
            .inner()
            .handle_update_now()
            .await
    ...
    Err("update manager unavailable".to_string())
}
...
/// Copy a frame deeplink (mrnobrainer://frame/N) to clipboard. Native API only.
#[tauri::command]
#[specta::specta]
pub async fn copy_deeplink_to_clipboard(frame_id: i64) -> Result<(), String> {
    let link = format!("mrnobrainer://frame/{}", frame_id);
    let mut clipboard = arboard::Clipboard::new().map_err(|e| format!("clipboard error: {}", e))?;
    clipboard
        .set_text(link)
        .map_err(|e| format!("failed to set clipboard: {}", e))?;
