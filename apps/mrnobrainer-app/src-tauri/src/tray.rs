...
fn create_dynamic_menu(
    ...
    if !onboarding_completed {
        menu_builder = menu_builder
            ...
            .item(&PredefinedMenuItem::separator(app)?)
            .item(
                &MenuItemBuilder::with_id("quit", crate::brand::QUIT_PRODUCT_LABEL).build(app)?,
            );
    ...
    menu_builder = menu_builder
        .item(
            &MenuItemBuilder::with_id("settings_top", crate::brand::OPEN_PRODUCT_MENU_LABEL)
                .build(app)?,
        )
        .item(&PredefinedMenuItem::separator(app)?);
    ...
    menu_builder = menu_builder.item(
        &MenuItemBuilder::with_id("build_info", "Founder alpha")
            .enabled(false)
            .build(app)?,
    );
    ...
    let version_text = if is_beta {
        format!("{} v{} (Beta)", crate::brand::PRODUCT_NAME, app.package_info().version)
    } else {
        format!("{} v{}", crate::brand::PRODUCT_NAME, app.package_info().version)
    };
    menu_builder = menu_builder.item(
    ...
    menu_builder = menu_builder
        ...
        .item(
            &MenuItemBuilder::with_id("quit", crate::brand::QUIT_PRODUCT_LABEL)
    ...
    match event.id().as_ref() {
        ...
        "releases" => {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                let _ = app.opener().open_url(
                    crate::brand::PRODUCT_RELEASES_URL,
                    None::<&str>,
                );
            });
        }
        "update_now" => {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                if let Some(manager) = app.try_state::<Arc<crate::updates::UpdatesManager>>() {
                    if let Err(error) = manager.inner().handle_update_now().await {
                        error!("failed to handle update menu action: {}", error);
                    }
                    return;
                }
            ...
            if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                if let Some(tray) = app_for_thread.tray_by_id("screenpipe_main") {
                    ...
                    let tooltip = if has_perm_issue {
                        format!("{} — ⚠️ permissions needed", crate::brand::PRODUCT_NAME)
                    } else {
                        crate::brand::PRODUCT_NAME.to_string()
                    };
