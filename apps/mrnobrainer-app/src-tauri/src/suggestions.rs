...
async fn fetch_app_activity() -> Result<Vec<AppActivity>, String> {
    ...
    let resp = client
        ...
        .json(&serde_json::json!({
            "query": "SELECT app_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'screenpipe' AND app_name != 'screenpipe-app' AND app_name != 'MRnObrainer' AND app_name != 'mrnobrainer-app' GROUP BY app_name ORDER BY cnt DESC LIMIT 15"
        }))
...
async fn fetch_window_activity() -> Result<Vec<WindowActivity>, String> {
    ...
    let resp = client
        .post(format!("{}/raw_sql", API))
        .json(&serde_json::json!({
            "query": "SELECT app_name, window_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'screenpipe' AND app_name != 'screenpipe-app' AND app_name != 'MRnObrainer' AND app_name != 'mrnobrainer-app' AND window_name != '' GROUP BY app_name, window_name ORDER BY cnt DESC LIMIT 20"
        }))
        .timeout(std::time::Duration::from_secs(10))
        .send()
        .await
...
mod tests {
    ...
    // ─── Benchmark tests ─────────────────────────────────────────────────────
    // Run with: cargo test -p mrnobrainer-app -- --ignored benchmark --nocapture
