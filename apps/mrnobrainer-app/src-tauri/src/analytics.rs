...
fn analytics_interval(interval_hours: u64) -> Duration {
    Duration::from_secs(interval_hours.saturating_mul(60 * 60))
}
...
impl AnalyticsManager {
    pub fn new(
        ...
        Self {
            ...
            distinct_id,
            email,
            interval: analytics_interval(interval_hours),
    ...
    pub async fn fetch_attribution(&self) {
        ...
        let Some(attribution_url) = self.attribution_url.as_ref() else {
            return;
        };
    ...
    pub async fn send_event(
        ...
        let mut payload = json!({
            ...
            "properties": {
                ...
                "release": format!("mrnobrainer-app@{}", app_version),
                "$set": {
                    "app_version": app_version,
                    "os_name": os_name,
                    "os_version": os_version,
...
pub fn start_analytics(
    ...
    let should_enable_analytics =
        analytics_enabled && !is_debug && !cfg!(debug_assertions) && !posthog_api_key.is_empty();
...
mod tests {
    ...
    #[test]
    fn test_interval_hours_are_converted_to_seconds() {
        assert_eq!(analytics_interval(6), Duration::from_secs(6 * 60 * 60));
    }
}
