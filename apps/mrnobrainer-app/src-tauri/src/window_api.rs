...
#[cfg(target_os = "macos")]
static MAIN_WINDOW_EXPLICIT_HIDE: AtomicBool = AtomicBool::new(false);
...
impl RewindWindowId {
    ...
    pub fn title(&self) -> &str {
        match self {
            RewindWindowId::Main => crate::brand::PRODUCT_NAME,
            RewindWindowId::Settings => crate::brand::PRODUCT_SETTINGS_WINDOW_TITLE,
            RewindWindowId::Search => crate::brand::PRODUCT_SEARCH_WINDOW_TITLE,
            RewindWindowId::Onboarding => crate::brand::PRODUCT_ONBOARDING_WINDOW_TITLE,
            RewindWindowId::Chat => crate::brand::PRODUCT_ASSISTANT_WINDOW_TITLE,
            RewindWindowId::PermissionRecovery => crate::brand::PRODUCT_PERMISSIONS_WINDOW_TITLE,
        }
    }
...
impl ShowRewindWindow {
                ...
                run_on_main_thread_safe(app, move || {
                    ...
                    if let Ok(panel) = app_clone.get_webview_panel(&lbl) {
                        ...
                        unsafe {
                            ...
                            // Explicit user intent path: when the overlay is reopened
                            // from shortcut, tray, or second-instance handoff, we must
                            // activate the app or the non-activating panel can remain
                            // behind the current frontmost app/Space even after
    ...
    pub fn show(&self, app: &AppHandle) -> tauri::Result<WebviewWindow> {
        ...
        if id.label() == RewindWindowId::Main.label() {
            #[cfg(target_os = "macos")]
            MAIN_WINDOW_EXPLICIT_HIDE.store(false, Ordering::SeqCst);
        ...
        let window = match self {
                ...
                window.on_window_event(move |event| {
                    match event {
                        ...
                        tauri::WindowEvent::Focused(is_focused) => {
                            if !is_focused {
                                #[cfg(target_os = "macos")]
                                if MAIN_WINDOW_EXPLICIT_HIDE.load(Ordering::SeqCst) {
                                    info!("Main window focus loss came from explicit hide; skipping debounce handler");
                                    return;
                                }
                                ...
                                #[cfg(target_os = "macos")]
                                if MAIN_WINDOW_EXPLICIT_HIDE.load(Ordering::SeqCst) {
                                    info!("Ignoring transient focus regain while explicit hide is in progress");
                                    return;
                                }
            ...
            ShowRewindWindow::Settings { page } => {
                let url = match page {
                    ...
                    None => "/settings?section=rewind".to_string(),
                };
    ...
    pub fn close(&self, app: &AppHandle) -> tauri::Result<()> {
        ...
        if id.label() == RewindWindowId::Main.label() {
            #[cfg(target_os = "macos")]
            {
                MAIN_WINDOW_EXPLICIT_HIDE.store(true, Ordering::SeqCst);
                ...
                std::thread::spawn(move || {
                    let _ = crate::commands::unregister_window_shortcuts(app_for_shortcuts);
                });
