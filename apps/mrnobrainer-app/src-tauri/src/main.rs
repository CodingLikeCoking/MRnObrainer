    ...
    unsafe {
        ...
        extern "C" fn dock_menu(_this: &Object, _sel: Sel, _sender: id) -> id {
            unsafe {
                ...
                let title = NSString::alloc(nil).init_str(crate::brand::OPEN_PRODUCT_MENU_LABEL);
...
struct OraclePairingContext {
    server_url: String,
    oracle_device_id: String,
    oracle_device_name: String,
}

fn resolve_local_oracle_ip() -> String {
...
#[tauri::command]
#[specta::specta]
fn get_oracle_pairing_context() -> OraclePairingContext {
    let hostname = hostname::get()
        .ok()
        .and_then(|name| name.into_string().ok())
        .filter(|name| !name.trim().is_empty())
        .unwrap_or_else(|| "Oracle Mac".to_string());
    let local_ip = resolve_local_oracle_ip();
    let oracle_device_id = hostname
        .to_lowercase()
        .chars()
        .map(|ch| if ch.is_ascii_alphanumeric() { ch } else { '-' })
        .collect::<String>();

    OraclePairingContext {
        server_url: format!("http://{}:3030", local_ip),
        oracle_device_id,
        oracle_device_name: hostname,
    }
}
...
async fn main() {
    ...
    // zbus/tokio conflict) and acts as a fallback on macOS/Windows.
    {
        ...
        let deep_link_url = args
            .iter()
            .find(|a| a.starts_with("mrnobrainer://") || a.starts_with("screenpipe://"))
            .cloned();
            ...
            .await
        {
            if resp.status().is_success() {
                eprintln!(
                    "{}: another instance is already running — focused existing window, exiting.",
    ...
    let telemetry_disabled = dirs::home_dir()
        ...
        .map(|enabled| !enabled)
        .unwrap_or(!store::default_analytics_enabled());

    let app_version = env!("CARGO_PKG_VERSION");
    let sentry_dsn = optional_env("MRNOBRAINER_SENTRY_DSN");
    let sentry_guard = if !telemetry_disabled {
        if let Some(sentry_dsn) = sentry_dsn.clone() {
            Some(sentry::init((
                sentry_dsn,
                sentry::ClientOptions {
                    release: Some(format!("mrnobrainer-app@{}", app_version).into()),
                    send_default_pii: false,
                    server_name: Some("mrnobrainer-app".into()),
            ...
            info!("Sentry disabled: MRNOBRAINER_SENTRY_DSN not configured");
            None
        }
    } else {
        None
    ...
    #[cfg(debug_assertions)]
    {
        ...
        let builder = Builder::new()
            .commands(collect_commands![
                ...
                commands::is_source_build_cmd,
                commands::get_default_settings_store,
    ...
    let app = app.manage(recording_state)
        ...
        .setup(move |app| {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                let app_submenu_builder = SubmenuBuilder::new(app, crate::brand::PRODUCT_NAME)
                    .item(&PredefinedMenuItem::about(
                        app,
                        Some(crate::brand::ABOUT_PRODUCT_LABEL),
                        None,
                    )?)
                    .separator();
                let app_submenu = app_submenu_builder
                    ...
                    .item(&PredefinedMenuItem::quit(
                        app,
                        Some(crate::brand::QUIT_PRODUCT_LABEL),
                    )?)
                    .build()?;
            ...
            let file_appender = RollingFileAppender::builder()
                ...
                .filename_prefix("mrnobrainer-app")
                .filename_suffix("log")
                .max_log_files(5)
                .build(log_dir)?;
            ...
            let posthog_api_key = optional_env("MRNOBRAINER_POSTHOG_API_KEY");
            let posthog_host = optional_env("MRNOBRAINER_POSTHOG_HOST")
                .unwrap_or_else(|| "https://eu.i.posthog.com".to_string());
            let attribution_url = optional_env("MRNOBRAINER_ATTRIBUTION_URL");
            ...
            let updates_manager = match updates::start_update_check(&app_handle, 360) {
                Ok(manager) => {
                    app.manage(manager.clone());
                    Some(manager)
                }
                Err(error) => {
                    error!("Failed to start update manager: {}", error);
                    None
                }
            };
            ...
            if !use_dev_mode {
                ...
                let app_for_delivery = app_handle.clone();
                ...
                std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        server_runtime.block_on(async move {
                            ...
                            match embedded_server::start_embedded_server(
                                config,
                                on_pipe_output,
                                app_for_delivery.clone(),
                            )
                            .await
                            {
                                Ok(handle) => {
            ...
            if let Some(_) = app_handle.tray_by_id("screenpipe_main") {
                let update_item = updates_manager
                    .as_ref()
                    .and_then(|manager| manager.update_now_menu_item_ref());
            ...
            if is_analytics_enabled {
                if let Some(posthog_api_key) = posthog_api_key.clone() {
                    ...
                    info!("PostHog disabled: MRNOBRAINER_POSTHOG_API_KEY not configured");
                }
            }
