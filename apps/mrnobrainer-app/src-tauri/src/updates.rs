...
use crate::brand::{
    PRODUCT_BETA_UPDATER_URL, PRODUCT_NAME, PRODUCT_RELEASES_URL, PRODUCT_STABLE_UPDATER_URL,
};
...
const GITHUB_RELEASES_URL: &str = PRODUCT_RELEASES_URL;
// ---------------------------------------------------------------------------
...
pub async fn install_specific_version(app: &tauri::AppHandle, version: &str) -> Result<(), String> {
    ...
    let rollback_base = std::env::var("MRNOBRAINER_ROLLBACK_BASE_URL")
        .ok()
        .map(|value| value.trim().trim_end_matches('/').to_string())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| {
            format!(
                "rollback endpoint is not configured. Set MRNOBRAINER_ROLLBACK_BASE_URL or download an older build from {}",
                GITHUB_RELEASES_URL
            )
        })?;
    let rollback_url = format!("{}/{}/{}", rollback_base, target_arch, version);
...
fn official_updater_enabled(app: &tauri::AppHandle) -> bool {
    !is_enterprise_build(app)
        && !is_source_build(app)
        && updater_pubkey_configured(app)
        && configured_updater_endpoint("stable").is_some()
}

fn resolve_update_channel(app: &tauri::AppHandle) -> String {
...
fn configured_updater_endpoint(channel: &str) -> Option<String> {
    let env_key = if channel.eq_ignore_ascii_case("beta") {
        "MRNOBRAINER_BETA_UPDATER_URL"
    } else {
        "MRNOBRAINER_STABLE_UPDATER_URL"
    };
    ...
    env_override.or_else(|| {
        let default = if channel.eq_ignore_ascii_case("beta") {
            PRODUCT_BETA_UPDATER_URL
        } else {
            PRODUCT_STABLE_UPDATER_URL
        };
...
impl UpdatesManager {
    pub fn new(app: &tauri::AppHandle, interval_minutes: u64) -> Result<Self, Error> {
        let update_menu_item = if is_enterprise_build(app) {
            ...
            let label = if official_updater_enabled(app) {
                "Check for updates"
            } else {
                "Open download page"
            };
            Some(
                MenuItemBuilder::with_id("update_now", label)
                    .enabled(true)
                    .build(app)?,
            )
        };
    ...
    pub async fn check_for_updates(
        ...
        if is_source_build(&self.app) {
            ...
            if show_dialog {
                self.show_manual_update_dialog().await?;
            }
            return Result::Ok(false);
        }
        ...
        if !official_updater_enabled(&self.app) {
            debug!("official updater unavailable; falling back to the manual download page");
            if let Some(ref item) = self.update_menu_item {
                item.set_enabled(true)?;
                item.set_text("Open download page")?;
            }
        ...
        let update_channel = resolve_update_channel(&self.app);
        let Some(updater_endpoint) = configured_updater_endpoint(&update_channel) else {
            debug!(
                "official updater unavailable: {} not configured",
        ...
        info!(
            "checking for updates via Tauri updater... (app={}, version={}, identifier={}, endpoint={})",
            product_name,
            current_version,
        ...
        if let Some(update) = check_result? {
            *self.update_available.lock().await = true;
            let available_info = serde_json::json!({
                "version": update.version,
                "body": update.body.clone().unwrap_or_default(),
                "stage": "available",
            });
            let _ = self.app.emit("update-available", available_info);
            ...
            if let Some(ref item) = self.update_menu_item {
                ...
                item.set_text(&format!("Downloading latest version of {}", PRODUCT_NAME))?;
            }
                ...
                match download_result {
                    Ok(_) => {
                        *self.update_installed.lock().await = true;
                        ...
                        let _ = self.app.emit("update-ready", ready_info);
                    }
                    Err(e) => {
                            ...
                            || err_str.contains("Forbidden")
                        {
                            ...
                            let _ = std::thread::spawn(move || {
                                let _ = app_notif
                                    .notification()
                                    .builder()
                                    .title(&format!("{} update available", PRODUCT_NAME))
                                    .body(format!(
            ...
            std::thread::spawn(move || {
                if let Err(e) = app_notif
                    .notification()
                    .builder()
                    .title(&format!("{} update ready", PRODUCT_NAME))
                    .body(format!("v{} downloaded — restart when ready", version_str))
                    .show()
                {
            ...
            if show_dialog {
                ...
                let update_dialog = self
                    ...
                    .title(&format!("{} update", PRODUCT_NAME))
        ...
        if show_dialog {
            self.app
                ...
                .title(&format!("{} is up to date", PRODUCT_NAME))
    ...
    /// Show dialog explaining in-app updates are unavailable for this build.
    async fn show_manual_update_dialog(&self) -> Result<(), Box<dyn std::error::Error>> {
        let (tx, rx) = oneshot::channel();
        let message = if is_source_build(&self.app) {
            format!(
                "{} installs updates from the manual download page in source builds.\n\n\
                Open the manual download page to get the latest stable or beta app bundle.",
                PRODUCT_NAME
            )
        } else {
            format!(
                "{} could not initialize the signed updater for this build.\n\n\
                Open the manual download page to get the latest stable or beta app bundle.",
                PRODUCT_NAME
            )
        };
        ...
        if open_releases {
            ...
            let _ = ShowRewindWindow::Settings { page: None }.show(&self.app);
        }

        Ok(())
    }

    pub async fn handle_update_now(&self) -> Result<(), Box<dyn std::error::Error>> {
        if *self.update_installed.lock().await {
            if let Err(err) =
                stop_screenpipe(self.app.state::<RecordingState>(), self.app.clone()).await
            {
                error!("Failed to stop recording before restart: {}", err);
