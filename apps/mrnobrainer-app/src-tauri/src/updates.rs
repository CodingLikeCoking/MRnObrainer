...
use crate::brand::{PRODUCT_NAME, PRODUCT_RELEASES_URL};
...
const STABLE_UPDATER_URL: &str = "https://screenpi.pe/api/app-update/latest.json";
const BETA_UPDATER_URL: &str = "https://screenpi.pe/api/app-update/beta.json";
...
fn official_updater_enabled(app: &tauri::AppHandle) -> bool {
    !is_enterprise_build(app) && !is_source_build(app) && updater_pubkey_configured(app)
}
...
fn updater_endpoint_for_channel(channel: &str) -> &'static str {
...
impl UpdatesManager {
    ...
    pub async fn check_for_updates(
        ...
        if !official_updater_enabled(&self.app) {
            debug!("official updater unavailable; falling back to GitHub Releases");
            if let Some(ref item) = self.update_menu_item {
                item.set_enabled(true)?;
                item.set_text("Open GitHub Releases")?;
            }
        ...
        let updater_endpoint = updater_endpoint_for_channel(&update_channel);
        let _ = self.app.emit(
            "update-checking",
            serde_json::json!({
    ...
    async fn show_manual_update_dialog(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let message = if is_source_build(&self.app) {
            format!(
                "{} installs updates from GitHub Releases in source builds.\n\n\
                Open the releases page to download the latest stable or beta app bundle.",
                PRODUCT_NAME
            )
        } else {
            format!(
