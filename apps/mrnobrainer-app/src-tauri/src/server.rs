...
fn sanitize_sensitive_url(raw: &str) -> String {
    let pattern =
        regex::Regex::new(r"(?i)(api_key|token|invite_token|key|license_key)=([^&]+)").unwrap();
    pattern.replace_all(raw, "$1=[REDACTED]").to_string()
}

fn allowed_local_origins() -> Vec<HeaderValue> {
    let mut origins = vec![
        "tauri://localhost",
    ...
    if let Ok(extra) = env::var("MRNOBRAINER_ALLOWED_ORIGINS") {
        origins.extend(
            extra
                .split(',')
                .map(str::trim)
...
async fn handle_focus(
    ...
    let sanitized_link = payload.deep_link_url.as_deref().map(sanitize_sensitive_url);
    info!(
        "Received focus request from second instance: args={:?}, deep_link={:?}",
        payload.args, sanitized_link
    );
