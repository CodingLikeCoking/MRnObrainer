...
if (IS_MACOS) {
  ...
  await test("tray icon visible", async () => {
    const result = await bb("find", "role:AXMenuBarItem AND name~:status", "--app", "mrnobrainer-app");
  ...
  await test("tray menu opens", async () => {
    ...
    try {
      await bb("click", "role:AXMenuBarItem AND name~:status", "--app", "mrnobrainer-app");
      await sleep(1000);
      // Verify menu items appeared
      const items = await bb("find", "role:AXMenuItem", "--app", "mrnobrainer-app");
  ...
  await test("tray 'show screenpipe' opens window", async () => {
    ...
    try {
      await bb("click", "role:AXMenuItem AND title~:show screenpipe", "--app", "mrnobrainer-app");
      await sleep(2000);

      // Verify window appeared
      const result = await bb("find", "role:AXWebArea", "--app", "mrnobrainer-app");
  ...
  await test("tray menu does not fire twice (S2.6)", async () => {
    await bb("click", "role:AXMenuBarItem AND name~:status", "--app", "mrnobrainer-app");
    await sleep(500);
    const result = await bb("find", "role:AXMenuItem AND title:settings", "--app", "mrnobrainer-app");
