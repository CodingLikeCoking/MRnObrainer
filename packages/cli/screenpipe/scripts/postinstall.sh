...
# Optional install tracking for internal builds only.
if [ -n "${SCREENPIPE_NPM_POSTINSTALL_POSTHOG_KEY:-}" ]; then
    SCREENPIPE_NPM_POSTINSTALL_HOST="${SCREENPIPE_NPM_POSTINSTALL_HOST:-https://eu.i.posthog.com}"
    curl -sL -X POST "${SCREENPIPE_NPM_POSTINSTALL_HOST}/capture/" \
        -H "Content-Type: application/json" \
        -d '{
            "api_key": "'"$SCREENPIPE_NPM_POSTINSTALL_POSTHOG_KEY"'",
