...
const TARGET_RECORDING_BACKLOG_SECS: u64 = 120;
const MIN_RECORDING_QUEUE_CAPACITY: usize = 2;
const MAX_RECORDING_QUEUE_CAPACITY: usize = 48;
const TRANSCRIPTION_QUEUE_CAPACITY: usize = 128;

fn estimated_recording_producer_count(enabled_devices: usize, use_all_devices: bool) -> usize {
    let configured_devices = enabled_devices.max(1);
    if use_all_devices {
        configured_devices.max(2)
    } else {
        configured_devices
    }
}

fn recording_queue_capacity(
    audio_chunk_duration: Duration,
    enabled_devices: usize,
    use_all_devices: bool,
) -> usize {
    let chunk_secs = audio_chunk_duration.as_secs().max(1);
    let per_device_capacity = TARGET_RECORDING_BACKLOG_SECS.div_ceil(chunk_secs) as usize;
    let capacity =
        per_device_capacity * estimated_recording_producer_count(enabled_devices, use_all_devices);
    capacity.clamp(MIN_RECORDING_QUEUE_CAPACITY, MAX_RECORDING_QUEUE_CAPACITY)
}
...
impl AudioManager {
    pub async fn new(options: AudioManagerOptions, db: Arc<DatabaseManager>) -> Result<Self> {
        ...
        let recording_queue_capacity = recording_queue_capacity(
            options.audio_chunk_duration,
            options.enabled_devices.len(),
            options.use_all_devices,
        );
        let (recording_sender, recording_receiver) =
            crossbeam::channel::bounded(recording_queue_capacity);
        let (transcription_sender, transcription_receiver) =
            crossbeam::channel::bounded(TRANSCRIPTION_QUEUE_CAPACITY);
...
mod tests {
    ...
    #[test]
    fn test_recording_queue_capacity_scales_with_chunk_duration() {
        assert_eq!(
            recording_queue_capacity(Duration::from_secs(10), 1, false),
            12
        );
        assert_eq!(
            recording_queue_capacity(Duration::from_secs(30), 1, false),
            4
        );
        assert_eq!(
            recording_queue_capacity(Duration::from_secs(120), 1, false),
            2
        );
    }

    #[test]
    fn test_recording_queue_capacity_scales_with_device_count() {
        assert_eq!(
            recording_queue_capacity(Duration::from_secs(10), 2, false),
            24
        );
        assert_eq!(
            recording_queue_capacity(Duration::from_secs(30), 3, false),
            12
        );
        assert_eq!(
            recording_queue_capacity(Duration::from_secs(10), 0, true),
            24
        );
    }
}
