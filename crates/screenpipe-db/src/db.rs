...
impl DatabaseManager {
    pub async fn new(database_path: &str) -> Result<Self, sqlx::Error> {
        ...
        let pool = SqlitePoolOptions::new()
            ...
            .max_connections(8)
            .min_connections(1) // Keep idle footprint low for laptop-scale OSS usage
    ...
    pub async fn delete_time_range(
        ...
        let snapshot_files: Vec<String> = sqlx::query_scalar(
            r#"SELECT DISTINCT snapshot_path FROM frames
               WHERE timestamp BETWEEN ?1 AND ?2
               AND video_chunk_id IS NULL
               AND snapshot_path IS NOT NULL
               AND snapshot_path NOT LIKE 'cloud://%'"#,
        )
        .bind(&start_str)
        .bind(&end_str)
        .fetch_all(&mut **tx.conn())
...
"#,
    ...
    pub async fn insert_accessibility_text(
        ...
        self.insert_accessibility_text_with_sync(
            app_name,
            window_name,
            text_content,
            browser_url,
    ...
    pub async fn insert_accessibility_text_with_sync(
        &self,
        app_name: &str,
        window_name: &str,
        text_content: &str,
        ...
        let result = sqlx::query(
            r#"
            INSERT INTO accessibility (app_name, window_name, text_content, browser_url, machine_id, sync_id)
            VALUES (?1, ?2, ?3, ?4, ?5, ?6)
            "#,
        )
        .bind(app_name)
    ...
    pub async fn create_cross_device_pairing_token(
        &self,
        label: &str,
        token_hash: &str,
        token_preview: &str,
    ) -> Result<CrossDevicePairingToken, sqlx::Error> {
        let mut tx = self.begin_immediate_with_retry().await?;

        let result = sqlx::query(
            r#"
            INSERT INTO cross_device_pairing_tokens (label, token_hash, token_preview)
            VALUES (?1, ?2, ?3)
            "#,
        )
        .bind(label)
        ...
        let token = sqlx::query_as::<_, CrossDevicePairingToken>(
            r#"
            SELECT id, label, token_hash, token_preview, created_at, revoked_at, last_used_at
            FROM cross_device_pairing_tokens
            WHERE id = ?1
            "#,
        )
    ...
    pub async fn list_cross_device_pairing_tokens(
        &self,
    ) -> Result<Vec<CrossDevicePairingToken>, sqlx::Error> {
        sqlx::query_as::<_, CrossDevicePairingToken>(
            r#"
            SELECT id, label, token_hash, token_preview, created_at, revoked_at, last_used_at
            FROM cross_device_pairing_tokens
    ...
    pub async fn touch_cross_device_pairing_token(
        &self,
        token_hash: &str,
    ) -> Result<Option<CrossDevicePairingToken>, sqlx::Error> {
        let mut tx = self.begin_immediate_with_retry().await?;

        let result = sqlx::query(
            r#"
            UPDATE cross_device_pairing_tokens
    ...
    pub async fn revoke_cross_device_pairing_token(
        &self,
        id: i64,
    ) -> Result<Option<CrossDevicePairingToken>, sqlx::Error> {
        let mut tx = self.begin_immediate_with_retry().await?;
