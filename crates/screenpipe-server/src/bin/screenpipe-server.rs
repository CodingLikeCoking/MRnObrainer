...
async fn main() -> anyhow::Result<()> {
    ...
    // Initialize Sentry only if telemetry is enabled and a DSN is explicitly configured.
    let sentry_dsn = env::var("SCREENPIPE_SENTRY_DSN")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());
    let _sentry_guard = if !cli.disable_telemetry {
        if let Some(sentry_dsn) = sentry_dsn {
            let sentry_release_name_append =
                env::var("SENTRY_RELEASE_NAME_APPEND").unwrap_or_default();
        ...
        sentry::configure_scope(|scope| {
            ...
            info!("Sentry disabled: SCREENPIPE_SENTRY_DSN not configured");
            None
        }
    } else {
        None
