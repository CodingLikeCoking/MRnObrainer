...
mod pii_redaction_tests {
    ...
    fn create_test_jpeg() -> Vec<u8> {
        // Create a 100x100 image with visible structure so blur-based redaction changes pixels.
        let img: ImageBuffer<Rgb<u8>, Vec<u8>> = ImageBuffer::from_fn(100, 100, |x, y| {
            let base = ((x * 3 + y * 5) % 256) as u8;
            let accent = if (20..50).contains(&x) && (20..40).contains(&y) {
                220
            } else {
                35
            };
            Rgb([base, 255u8.saturating_sub(base), accent])
        });
