...
impl HotFrameCache {
    fn earliest_frame_timestamp(
        frames: &BTreeMap<(DateTime<Utc>, i64), HotFrame>,
    ) -> Option<DateTime<Utc>> {
        frames.keys().next().map(|(timestamp, _)| *timestamp)
    }

    pub fn default_warm_hours() -> i64 {
        if cfg!(feature = "oss-public") {
            6
        } else {
            24
        }
    ...
    pub fn new() -> Self {
        let max_frame_entries = if cfg!(feature = "oss-public") {
            2_500
        } else {
            10_000
        };
        let max_audio_entries = if cfg!(feature = "oss-public") {
            1_000
        } else {
            4_000
        };
        Self::new_with_limits(max_frame_entries, max_audio_entries)
    }

    pub fn new_with_limits(max_frame_entries: usize, max_audio_entries: usize) -> Self {
        ...
        Self {
            ...
            max_frame_entries: max_frame_entries.max(1),
            max_audio_entries: max_audio_entries.max(1),
    ...
    fn prune_frames_locked(&self, frames: &mut BTreeMap<(DateTime<Utc>, i64), HotFrame>) {
        while frames.len() > self.max_frame_entries {
            let oldest = frames.keys().next().copied();
    ...
    fn prune_audio_locked(&self, audio: &mut BTreeMap<DateTime<Utc>, Vec<HotAudio>>) {
        let mut total_entries = audio.values().map(Vec::len).sum::<usize>();
        while total_entries > self.max_audio_entries {
            let oldest = audio.keys().next().copied();
            if let Some(oldest) = oldest {
                if let Some(removed) = audio.remove(&oldest) {
                    total_entries = total_entries.saturating_sub(removed.len());
                }
            } else {
                break;
            }
    ...
    pub async fn push_frame(&self, frame: HotFrame) {
        ...
        self.prune_frames_locked(&mut frames);
        let earliest_coverage = Self::earliest_frame_timestamp(&frames);
        drop(frames);
        *self.cache_warm_start.write().await = earliest_coverage;
    ...
    pub async fn push_audio(&self, audio: HotAudio) {
        ...
        audio_map
            ...
            .or_default()
            .push(audio.clone());
        self.prune_audio_locked(&mut audio_map);
        drop(audio_map);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_prunes_oldest_frames_when_limit_exceeded() {
        let cache = HotFrameCache::new_with_limits(2, 4);
        let base = Utc::now();
        ...
        for i in 0..3 {
            cache
                .push_frame(HotFrame {
                    ...
                    window_name: "Win".to_string(),
                    ocr_text_preview: String::new(),
                    snapshot_path: format!("/tmp/{i}.jpg"),
                    browser_url: None,
                    capture_trigger: "click".to_string(),
                    offset_index: i,
                    fps: 0.033,
        ...
        assert_eq!(result[0].frame_data[0].frame_id, 1);
        assert_eq!(result[1].frame_data[0].frame_id, 2);
        assert_eq!(
    ...
    #[tokio::test]
    async fn test_warm_coverage_tracks_retained_frames_after_pruning() {
        let cache = HotFrameCache::new_with_limits(2, 4);
        let base = Utc::now();
