...
const ARCHIVE_CLEANUP_BATCH_HOURS: i64 = 6;

fn archive_batch_delete_end(exclusive_end: DateTime<Utc>) -> DateTime<Utc> {
    exclusive_end - Duration::nanoseconds(1)
}

async fn oldest_local_timestamp_before(
    db: &DatabaseManager,
    cutoff: DateTime<Utc>,
) -> anyhow::Result<Option<DateTime<Utc>>> {
    let cutoff_str = cutoff.to_rfc3339();
    let min_ts: Option<String> = sqlx::query_scalar(
        r#"
        SELECT MIN(ts) FROM (
            SELECT MIN(timestamp) AS ts FROM frames WHERE timestamp < ?1
            UNION ALL
            SELECT MIN(timestamp) AS ts FROM audio_transcriptions WHERE timestamp < ?1
            UNION ALL
            SELECT MIN(timestamp) AS ts FROM accessibility WHERE timestamp < ?1
            UNION ALL
            SELECT MIN(timestamp) AS ts FROM ui_events WHERE timestamp < ?1
...
async fn do_cleanup(db: &Arc<DatabaseManager>, cutoff: DateTime<Utc>) -> anyhow::Result<()> {
    let Some(mut batch_start) = oldest_local_timestamp_before(db, cutoff).await? else {
        return Ok(());
    };
    let mut frames_deleted = 0;
    let mut ocr_deleted = 0;
    let mut audio_deleted = 0;
    let mut accessibility_deleted = 0;
    let mut ui_events_deleted = 0;

    while batch_start < cutoff {
        let batch_end_exclusive = std::cmp::min(
            batch_start + Duration::hours(ARCHIVE_CLEANUP_BATCH_HOURS),
            cutoff,
        );
        let result = db
            .delete_time_range(batch_start, archive_batch_delete_end(batch_end_exclusive))
            .await?;

        frames_deleted += result.frames_deleted;
        ocr_deleted += result.ocr_deleted;
        audio_deleted += result.audio_transcriptions_deleted;
    ...
    info!(
        ...
        frames_deleted, ocr_deleted, audio_deleted, accessibility_deleted, ui_events_deleted,
    );
...
mod tests {
    ...
    #[test]
    fn archive_cleanup_uses_exclusive_cutoff_boundaries() {
        let cutoff = DateTime::parse_from_rfc3339("2026-03-11T12:00:00Z")
            .unwrap()
            .with_timezone(&Utc);

        assert_eq!(
            archive_batch_delete_end(cutoff),
