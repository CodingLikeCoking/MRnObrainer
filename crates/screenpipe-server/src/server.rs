...
use crate::{
    ...
    routes::{
        ...
        cross_device::{
            create_cross_device_token, ingest_cross_device_events, list_cross_device_tokens,
            revoke_cross_device_token,
        },
...
fn allowed_local_origins() -> Vec<axum::http::HeaderValue> {
    let mut origins = vec![
        "tauri://localhost",
    ...
    .into_iter()
    .map(|origin| axum::http::HeaderValue::from_str(origin).unwrap())
    .collect::<Vec<_>>();

    if let Ok(extra) = env::var("SCREENPIPE_ALLOWED_ORIGINS") {
        origins.extend(
            extra
                .split(',')
                .map(str::trim)
...
impl SCServer {
    ...
    pub async fn create_router(&self) -> Router {
            ...
            .unwrap_or_else(|| Arc::new(HotFrameCache::new()));
        {
            ...
            tokio::spawn(async move {
                cache
                    .warm_from_db(&db, HotFrameCache::default_warm_hours())
                    .await;
            });
        }
        ...
        let cors = CorsLayer::new()
            .allow_origin(AllowOrigin::list(allowed_local_origins()))
            .allow_methods([
        ...
        let router = Router::new()
            ...
            .route(
                "/cross-device/events",
                axum::routing::post(ingest_cross_device_events),
            )
            .route(
                "/cross-device/tokens",
                axum::routing::post(create_cross_device_token).get(list_cross_device_tokens),
            )
            .route(
                "/cross-device/tokens/:id/revoke",
                axum::routing::post(revoke_cross_device_token),
            );
