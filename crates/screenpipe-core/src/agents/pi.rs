...
const PI_SUBPROCESS_TIMEOUT: Duration = Duration::from_secs(300);
...
impl PiExecutor {
    ...
    pub fn ensure_pi_config(
        ...
        } else if let Some(providers) = models_config
            .get_mut("providers")
            .and_then(|p| p.as_object_mut())
        {
    ...
    async fn spawn_pi(
        ...
        let output =
            wait_for_child_output_with_timeout(child, PI_SUBPROCESS_TIMEOUT, "pi subprocess")
                .await?;
...
async fn wait_for_child_output_with_timeout(
    ...
    let mut stdout_handle = child
        .stdout
        .take()
        .ok_or_else(|| anyhow!("failed to capture {process_label} stdout"))?;
    let mut stderr_handle = child
        .stderr
        .take()
        .ok_or_else(|| anyhow!("failed to capture {process_label} stderr"))?;

    let stdout_task = tokio::spawn(async move {
        let mut stdout = Vec::new();
        tokio::io::AsyncReadExt::read_to_end(&mut stdout_handle, &mut stdout).await?;
        Ok::<Vec<u8>, std::io::Error>(stdout)
    });
    let stderr_task = tokio::spawn(async move {
        let mut stderr = Vec::new();
        tokio::io::AsyncReadExt::read_to_end(&mut stderr_handle, &mut stderr).await?;
    ...
    let status = match tokio::time::timeout(timeout, child.wait()).await {
        ...
        Err(_) => {
            if let Some(pid) = pid {
                ...
                warn!(
                    "{} timed out after {}s; terminating child without known pid",
                    process_label,
                    timeout.as_secs()
                );
    ...
    let stdout = stdout_task
        .await
        .map_err(|err| anyhow!("failed to join {process_label} stdout task: {err}"))??;
    let stderr = stderr_task
        .await
        .map_err(|err| anyhow!("failed to join {process_label} stderr task: {err}"))??;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_wait_for_child_output_with_timeout_returns_prompt_error() {
        #[cfg(unix)]
        let mut cmd = {
        ...
        let mut cmd = {
            ...
            cmd.args(["/C", "timeout /T 30 /NOBREAK >NUL"]);
            cmd
        };
        ...
        let child = cmd.spawn().expect("test child should spawn");
        let started = Instant::now();
        let err = wait_for_child_output_with_timeout(
            child,
        ...
        assert!(
            err.to_string().contains("test pi subprocess timed out"),
