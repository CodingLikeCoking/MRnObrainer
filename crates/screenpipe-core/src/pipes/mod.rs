...
fn is_default_delivery_targets(targets: &[DeliveryTarget]) -> bool {
    targets == [DeliveryTarget::Dashboard]
}
...
pub struct PipeConfig {
    ...
    /// Where scheduled output should be delivered. Defaults to dashboard inbox.
    #[serde(
        default = "default_delivery_targets",
        rename = "deliveryTargets",
        alias = "delivery-targets",
        skip_serializing_if = "is_default_delivery_targets"
    )]
...
/// Rich run-finished event used by higher-level hosts (desktop app, etc.)
/// to deliver scheduled output without coupling the core runtime to UI code.
#[derive(Debug, Clone)]
pub struct PipeRunEvent {
    pub pipe_name: String,
    pub trigger: String,
...
/// Callback fired with the completed run payload.
pub type OnPipeRunEvent = Arc<dyn Fn(PipeRunEvent) + Send + Sync>;
...
pub struct PipeManager {
    ...
    /// Optional callback fired with the full completed run payload.
    on_run_event: Option<OnPipeRunEvent>,
...
impl PipeManager {
    ...
    /// Set a callback to be invoked with the full completed run payload.
    pub fn set_on_run_event(&mut self, cb: OnPipeRunEvent) {
        self.on_run_event = Some(cb);
    }
    ...
    pub async fn start_pipe_background(&self, name: &str) -> Result<()> {
        ...
        tokio::spawn(async move {
            ...
            if let Some(ref cb) = on_run_event {
                cb(PipeRunEvent {
                    pipe_name: name_for_cb,
        ...
        if source_path.exists() {
            ...
            if source_path.is_file() && source_path.extension().is_some_and(|e| e == "md") {
                ...
                let name = source_path
                    ...
                    .to_string_lossy()
                    .to_string();
                let pipe_md = source_path.join("pipe.md");
                if !pipe_md.exists() {
                    return Err(anyhow!(
                        "pipe directory '{}' must contain a pipe.md file",
                        source
                    ));
                }
                let content = std::fs::read_to_string(&pipe_md)?;
                parse_frontmatter(&content).map_err(|e| {
                    anyhow!("invalid pipe.md in '{}': {}", source_path.display(), e)
                })?;
                let dest_dir = self.pipes_dir.join(&name);
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (name, config, body) in &pipe_snapshot {
                    ...
                    let on_run_event = on_run_event.clone();
...
mod tests {
    ...
    fn test_parse_frontmatter_defaults() {
        ...
        assert_eq!(config.delivery_targets, vec![DeliveryTarget::Dashboard]);
    }

    #[test]
    fn test_parse_frontmatter_delivery_targets() {
        let content = "---\nschedule: every 1h\ndeliveryTargets:\n  - dashboard\n  - telegram\n---\n\nBody";
        let (config, _) = parse_frontmatter(content).unwrap();
        assert_eq!(
            config.delivery_targets,
            vec![DeliveryTarget::Dashboard, DeliveryTarget::Telegram]
        );
    }
    ...
    fn test_render_prompt_uses_port() {
        let config = PipeConfig {
            ...
            allow_raw_sql: false,
            allow_frames: true,
            delivery_targets: vec![DeliveryTarget::Dashboard],
            config: HashMap::new(),
        };
