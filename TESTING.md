...
## risk classifier evidence markers

For risky PRs, add one detailed `Risk-Evidence:` line per matched group in the PR body so CI can verify that the evidence was attached. Use this format:

- `Risk-Evidence: <marker> | <specific evidence details>`

- `Risk-Evidence: windowing | TESTING.md sections 1.1, 1.2; screenshot attached` for windowing, tray, fullscreen, or monitor-routing changes. Cite the exact checklist items used from sections 1 to 3 below.
- `Risk-Evidence: capture | redacted logs from permission recovery; capture health screenshot attached` for permissions, capture, relaunch, updater, or sync changes. Attach redacted screenshots or logs and reference the relevant checks below.
- `Risk-Evidence: oss_surface | reviewed public assets for redaction and branding consistency` for OSS-facing docs, screenshots, or fixtures. Confirm the review covered private-data redaction and branding consistency.
