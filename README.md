...
<p align="center">
  <a href="https://github.com/CodingLikeCoking/MRnObrainer/releases">Download</a>
  ·
...
1. Open the [official release page](https://github.com/CodingLikeCoking/MRnObrainer/releases).
2. Download the latest macOS app bundle for your machine.
...
1. Download the latest app bundle from the
   [official release page](https://github.com/CodingLikeCoking/MRnObrainer/releases).
...
That starts the **web UI only** on `localhost:1420`. For the full desktop app
with the Tauri shell, local capture backend, tray, shortcuts, and permissions
flow, use:
...
the web dev server through `beforeDevCommand`. Also do not run the standalone
`screenpipe-server record` backend at the same time as the desktop app, because
both want the local capture API port.

## Solo dev launch modes

Use one launch mode at a time:

| Goal | Command | Starts | Avoid running at the same time |
| --- | --- | --- | --- |
| UI-only iteration | `bun dev` from `apps/mrnobrainer-app` | Next.js on `localhost:1420` | `bun run tauri dev` |
| Full desktop app | `bun run tauri dev` from `apps/mrnobrainer-app` | Tauri, Next dev server, embedded capture API, tray, shortcuts | separate `bun dev`; standalone `screenpipe-server record` |
| Backend-only capture/API debugging | `cargo run -p screenpipe-server -- record` from repo root | standalone capture API on `localhost:3030` | desktop app capture backend |
| Automated UI smoke | `bun run browser-smoke` from `apps/mrnobrainer-app` | temporary Next server and Playwright route checks | another process on `localhost:1420` |

For a longer single-developer workflow, see
...
The public macOS app name is intentionally decoupled from the deeper app
identity.

- Change the public app name in
  `apps/mrnobrainer-app/brand.config.json`.
- That branding source-of-truth drives the app display name, window titles, and
  installer-facing naming.
- Do **not** change bundle identifier, updater identity, signing identity, or
  permission continuity as part of a normal rename.

This keeps `MRnObrainer.app` free to become another public-facing app name later
without breaking the installed app's permissions or update chain.
...
- [docs/architecture.md](docs/architecture.md): product architecture and system
  relationships
- [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md): solo developer launch modes,
  debugging, and safe vibe-coding workflow
